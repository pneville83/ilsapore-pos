...
router.post('/login', async (req, res) => {
    ...
    if (username === 'cocina' && password === 'cocina123') { return res.status(200).send({ message: 'Login exitoso', role: 'cocina' }); }
    if (username === 'mesero' && password === 'mesero123') { return res.status(200).send({ message: 'Login exitoso', role: 'mesero' }); }
    try {
...
router.get('/pedidos/activos', async (req, res) => { try { const query = `SELECT p.id, p.fecha, p.direccion_mz, p.direccion_villa, p.total, p.observaciones, p.estado, (SELECT json_agg(json_build_object('nombre', prod.nombre, 'cantidad', dp.cantidad, 'nombre_variacion', dp.nombre_variacion)) FROM detalles_pedido dp JOIN productos prod ON dp.producto_id = prod.id WHERE dp.pedido_id = p.id) as productos, (SELECT json_agg(json_build_object('cuenta', t.cuenta, 'monto', t.monto)) FROM transacciones t WHERE t.pedido_id = p.id AND t.tipo = 'Ingreso') as pagos FROM pedidos p WHERE p.estado != 'Finalizado' ORDER BY p.fecha ASC;`; const { rows } = await db.query(query); res.json(rows); } catch (err) { console.error('Error al obtener pedidos activos:', err); res.status(500).send('Error en el servidor'); } });
...
router.put('/finanzas/transaccion/:id', async (req, res) => {
    ...
    try {
        const query = `UPDATE transacciones SET descripcion = $1, monto = $2, cuenta = $3 WHERE id = $4 RETURNING *;`;
        const { rows } = await db.query(query, [descripcion, monto, cuenta, id]);
        if (rows.length === 0) return res.status(404).send('Transacción no encontrada');
        res.json(rows[0]);
