...
// --- La función printTicket y otras rutas no cambian, pero se incluyen para integridad ---
...
// --- RUTA DE CREACIÓN DE PEDIDOS (MODIFICADA) ---
router.post('/pedidos', async (req, res) => {
    ...
    try {
        ...
        // ¡MODIFICACIÓN! La consulta ahora incluye 'nombre_variacion'
        ...
        for (const producto of productos) {
            // Pasamos el nombre de la variación como el quinto parámetro
            await client.query(detallesQuery, [nuevoPedidoId, producto.producto_id, producto.cantidad, producto.precio_unitario, producto.nombre_variacion]);
        }
        ...
        const nuevoPedidoQuery = `SELECT p.id, p.fecha, p.direccion_mz, p.direccion_villa, p.total, p.observaciones, p.estado, (SELECT json_agg(json_build_object('nombre', prod.nombre, 'cantidad', dp.cantidad, 'nombre_variacion', dp.nombre_variacion)) FROM detalles_pedido dp JOIN productos prod ON dp.producto_id = prod.id WHERE dp.pedido_id = p.id) as productos, (SELECT json_agg(json_build_object('cuenta', t.cuenta, 'monto', t.monto)) FROM transacciones t WHERE t.pedido_id = p.id AND t.tipo = 'Ingreso') as pagos FROM pedidos p WHERE p.id = $1;`;
...
// --- RUTA DE PEDIDOS ACTIVOS (MODIFICADA) ---
router.get('/pedidos/activos', async (req, res) => {
    try {
        // ¡MODIFICACIÓN! La consulta ahora también trae los pagos y el nombre de la variación
        const query = `
            SELECT 
                ...
                (SELECT json_agg(json_build_object(
                    ...
                    'nombre_variacion', dp.nombre_variacion
                )) 
                ...
                WHERE t.pedido_id = p.id AND t.tipo = 'Ingreso') as pagos
            FROM pedidos p
...
router.get('/reportes/cierre-caja', async (req, res) => { const { fecha_inicio, fecha_fin } = req.query; try { const query = `SELECT cuenta, COALESCE(SUM(CASE WHEN tipo = 'Ingreso' THEN monto ELSE 0 END), 0) as ingresos, COALESCE(SUM(CASE WHEN tipo = 'Egreso' THEN monto ELSE 0 END), 0) as gastos FROM transacciones WHERE (fecha AT TIME ZONE $3)::date BETWEEN $1::date AND $2::date GROUP BY cuenta;`; const { rows } = await db.query(query, [fecha_inicio, fecha_fin, TIMEZONE]); const resultado = { 'Efectivo': { ingresos: 0, gastos: 0, balance: 0 }, 'Transferencia': { ingresos: 0, gastos: 0, balance: 0 }, 'Tarjeta': { ingresos: 0, gastos: 0, balance: 0 } }; rows.forEach(row => { if (resultado[row.cuenta]) { resultado[row.cuenta].ingresos = parseFloat(row.ingresos); resultado[row.cuenta].gastos = parseFloat(row.gastos); resultado[row.cuenta].balance = parseFloat(row.ingresos) - parseFloat(row.gastos); } }); res.json(resultado); } catch (err) { console.error('Error generando cierre de caja:', err); res.status(500).send('Error en el servidor'); } });
router.get('/reportes/productos-vendidos', async (req, res) => { const { fecha_inicio, fecha_fin } = req.query; try { const query = `SELECT p.nombre, SUM(dp.cantidad) as total_vendido, SUM(dp.cantidad * dp.precio_unitario) as ingresos_generados FROM productos p JOIN detalles_pedido dp ON p.id = dp.producto_id JOIN pedidos ped ON dp.pedido_id = ped.id WHERE (ped.fecha AT TIME ZONE $3)::date BETWEEN $1::date AND $2::date GROUP BY p.nombre ORDER BY total_vendido DESC;`; const { rows } = await db.query(query, [fecha_inicio, fecha_fin, TIMEZONE]); res.json(rows); } catch (err) { console.error('Error generando reporte de productos:', err); res.status(500).send('Error en el servidor'); } });
router.get('/reportes/direcciones', async (req, res) => { const { fecha_inicio, fecha_fin } = req.query; try { const query = `SELECT direccion_mz, direccion_villa, COUNT(id) as numero_pedidos, SUM(total) as total_consumido FROM pedidos WHERE (fecha AT TIME ZONE $3)::date BETWEEN $1::date AND $2::date GROUP BY direccion_mz, direccion_villa ORDER BY total_consumido DESC;`; const { rows } = await db.query(query, [fecha_inicio, fecha_fin, TIMEZONE]); res.json(rows); } catch (err) { console.error('Error generando reporte por dirección:', err); res.status(500).send('Error en el servidor'); } });
